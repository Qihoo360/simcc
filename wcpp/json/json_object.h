...
class JSONArray;
class JSONTokener;
class WCPP_EXPORT JSONObject : public Object, public JSONParser {
public:
...
public:
    ...
    JSONObject(const char* source, const wcpp::int32 source_len = -1);
...
public:
    ...
    wcpp::uint32 Parse(const std::string& source);
    ...
    // @return number of characters parsed. Return 0 if failed to parse.
    wcpp::uint32 Parse(const char* source, const wcpp::int64 source_len = -1);
    ...
    WCPP_EXPORT friend wcpp::DataStream& operator >> (wcpp::DataStream& file, JSONObject& val);

    // Save, Serialize. save the json object in the data stream
    WCPP_EXPORT friend wcpp::DataStream& operator << (wcpp::DataStream& file, const JSONObject& val);

#ifdef H_PROVIDE_PROTOBUF_DEBUGSTRING_2_JSON
    wcpp::uint32 ParseProtobuf(const std::string& protobuf_debug_string);
#endif
...
public:
    ...
    wcpp::float64 GetDecimal(const std::string& strKey, float64 default_value = 0.0) const;
    ...
    // @return true, if successfully
    //   or false, if failed
    bool PutByteArray(const std::string& key, const wcpp::int8* value, wcpp::uint32 count);
    bool PutInt32Array(const std::string& key, const wcpp::int32* value, wcpp::uint32 count);
    bool PutInt64Array(const std::string& key, const wcpp::int64* value, wcpp::uint32 count);
    bool PutFloat32Array(const std::string& key, const wcpp::float32* value, wcpp::uint32 count);
    bool PutFloat64Array(const std::string& key, const wcpp::float64* value, wcpp::uint32 count);
    bool PutBoolArray(const std::string& key, const bool* value, wcpp::uint32 count);
    bool PutStringArray(const std::string& key, const std::string* value, wcpp::uint32 count);
...
protected:
    // Produce a string in wcpp::f64 quotes with backslash sequences in all the
    ...
    static void Quote(const std::string& source, bool utf8_to_unicode, wcpp::DataStream& sb);

    template<class T>
    bool PutIntegerArray(const std::string& key, const T* value, wcpp::uint32 count);
...
typedef wcpp::RefPtr<JSONObject>  JSONObjectPtr;
typedef wcpp::RefPtr<JSONInteger> JSONIntegerPtr;
typedef wcpp::RefPtr<JSONDouble>  JSONDoublePtr;
typedef wcpp::RefPtr<JSONArray>   JSONArrayPtr;
typedef wcpp::RefPtr<JSONString>  JSONStringPtr;
typedef wcpp::RefPtr<JSONBoolean> JSONBooleanPtr;
typedef wcpp::RefPtr<JSONNull>    JSONNullPtr;
